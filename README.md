...
字幕生成后，脚本会自动调用 智谱 API 将字幕翻译为目标语言（默认中文）。
...
> 💡 **说明**：`D:\000\` 只是教程中举例用的路径，你可以换成任意位置（如 `D:\mpv-ai\`、`E:\tools\` 等），只要后续步骤中所有路径保持一致即可。
...
> **方式一：GitHub 单独下载**
>
> - 📥 [sub-fastwhisper.lua](https://github.com/dyphire/mpv-config/blob/master/scripts/sub-fastwhisper.lua)
> - 📥 [sub_fastwhisper.conf](https://github.com/dyphire/mpv-config/blob/master/script-opts/sub_fastwhisper.conf)
>
> **方式二：网盘打包下载（两个文件一起）**
>
